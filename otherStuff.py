...
class HelpMethods(object):
    ...
    def authorperm(self, ctx, perm):
        if eval(f'ctx.channel.permissions_for(ctx.author).{perm}'):
            return True
        return False
    
    def meperm(self, ctx, perm):
        me = ctx.guild.get_member(447268676702437376)
        if eval(f'ctx.channel.permissions_for(me).{perm}'):
            return True
        return False
    
    def serverauthorperm(self, ctx, perm):
        if eval(f'ctx.author.guild_permissions.{perm}'):
            return True
        return False
    
    def servermeperm(self, ctx, perm):
        me = ctx.guild.get_member(447268676702437376)
    ...
    async def noperms(self, ctx, *perms, send=True, server=False, annd=True):
        lackingperms = []
        melp = []
        for perm in perms:
            if not server:
                if not self.authorperm(ctx, perm):
                    lackingperms.append(perm)
                if not self.meperm(ctx, perm) and perm != 'manage_guild':
                    melp.append(perm)
            else:
                if not self.serverauthorperm(ctx, perm):
                    lackingperms.append(perm)
                if not self.servermeperm(ctx, perm) and perm != 'manage_guild':
                    melp.append(perm)
        if annd:
            tosend = ''
            metosend = ''
            ...
            if send and (tosend or metosend):
                return await ctx.channel.send(tosend+metosend)
            else:
                return tosend
        else:
            strperms = ', '.join(perms)
            ...
            if len(lackingperms) == len(perms):
                if len(perms) > 1:
                    tosend = f"You must have one of these permiss ions to use that command: `{strperms}`"
                else:
                    tosend = f"You must have the {strperms} permission to use that command."
            elif len(melp) != len(perms):
                return False
            if send:
                return await ctx.channel.send(tosend+metosend)
            else:
                return tosend
    async def noperm(self, ctx, *perms, send=True, server=False):
        await self.noperms(ctx, *perms, send=send, server=server, annd=False)
    ...
    def getchannel(self, guild, posschannel, byname=True):
        ...
        if posschannel.isdigit():
            ...
            channel = discord.utils.get(guild.channels, mention=posschannel)
            if not channel and byname:
                channel = discord.utils.get(guild.channels, name=posschannel)
        return channel
